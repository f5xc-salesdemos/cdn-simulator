...
title: 验证
description: >-
  使用预期的 HTTP 响应码、X-Cache-Status 头部值（HIT/MISS）、健康检查 JSON
  响应、缓存目录检查、源站连通性检查进行冒烟测试，以及 cloud-init、NGINX 启动和 NSG 问题的故障排除
sidebar:
  order: 5
i18n:
...
部署完成后，运行以下检查以确认 CDN 边缘节点正常运行。在 `terraform apply` 之后，请等待 2-3 分钟让 cloud-init 完成 NGINX 的安装。
...
对任何路径的首次请求将会是缓存未命中 — 边缘节点从源站获取内容：
...
`HIT` 确认响应是从 NGINX 磁盘缓存提供的，无需联系源站。

## 缓存目录检查

通过 SSH 登录虚拟机，验证磁盘上存在缓存内容：
...
通过请求 `/headers` 路径验证所有 CDN 供应商头部是否已注入（使用 httpbin.org 作为源站时）：
...
使用移动端 User-Agent 进行测试，验证设备检测头部是否发生变化：
...
在 `terraform apply` 之后，cloud-init 需要 2-3 分钟。检查进度：
...
- 验证源站返回可缓存的响应码（200、301、302）
- 检查源站是否发送了 `Cache-Control: no-cache` 或 `no-store` 头部
