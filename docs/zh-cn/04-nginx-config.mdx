...
cloud-init 配置部署了一套完全经过性能优化的 NGINX CDN 边缘配置。本页面记录了从内核调优到缓存行为再到厂商头部注入的每一个配置层。所有设置均经过 48 小时持续负载测试验证，峰值达到 172,540 req/s。
...
| `/etc/nginx/conf.d/cdn-edge.conf` | CDN 代理配置（缓存、上游、头部） |

## 内核调优

通过 `/etc/sysctl.d/99-cdn-tuning.conf` 在启动时应用。
...
| `net.ipv4.ip_local_port_range` | 1024-65535 | 32768-60999 | 临时端口范围（64K vs 28K） |
...
| `net.ipv4.tcp_slow_start_after_idle` | 0 | 1 | 在空闲连接上保持拥塞窗口热度 |
...
256k 的 busy buffer 大小至关重要——它必须超过最大的缓存响应（Juice Shop 为 75 KB）。最初的 64k 设置在高负载下导致了序列化问题。
...
缓存已缓存对象的文件描述符和元数据，消除了热点文件上的 `stat()` 和 `open()` 系统调用。
...
在 90K req/s 下使用 `keepalive_requests 1000` 时，连接每 11 秒就会回收一次，产生 TIME_WAIT 套接字。将其增加到 100,000 后完全消除了此问题。
...
4 个 worker x 256 keepalive = 1,024 个到源站的热连接。结合 `proxy_http_version 1.1` 和 `proxy_set_header Connection ""`，这消除了每次缓存未命中时的 TCP 握手开销。
...
| `max_size=25g` | 25 GB 磁盘限制 | 使用 30 GB 操作系统磁盘的大部分空间；达到限制时进行 LRU 淘汰 |
| `inactive=24h` | 24 小时不活跃超时 | 24 小时未被访问的内容将被淘汰（不同于 TTL） |
...
| 指令 | 值 | 用途 |
|---|---|---|
| `proxy_cache_valid 200 301 302 4h` | 4 小时 TTL | 缓存内容有效期为 4 小时（增加以减少刷新波导致的吞吐量下降） |
| `proxy_cache_lock on` | 惊群效应防护 | 每个未缓存的 URL 只有 1 个请求发送到源站；其他请求等待缓存填充 |
| `proxy_cache_lock_age 3s` | 锁超时 | 如果第一个请求在 3 秒内未完成，允许另一个请求通过 |
| `proxy_cache_background_update on` | 零延迟刷新 | 立即提供过期内容，同时在后台刷新 |
| `proxy_cache_use_stale` | 弹性机制 | 在源站错误（500/502/503/504）或更新期间提供过期内容 |
| `proxy_ignore_headers` | 强制缓存 | 忽略源站的 Set-Cookie、Cache-Control、Expires、Vary——由 CDN 决定 TTL 和 Vary 行为（Juice Shop 发送 `max-age=0` 和三重 `Vary` 头部，这会阻止有效缓存） |
| `proxy_hide_header X-Cache-Status` | 剥离源站头部 | 源站 NGINX 添加了自己的 X-Cache-Status——剥离它以便只显示 CDN 的缓存状态 |
| `proxy_hide_header Vary` | 防止缓存碎片化 | 源站发送多个 `Vary: Accept-Encoding` 头部。剥离它们可以防止缓存键在 `Accept-Encoding` 排列组合间碎片化。NGINX 的 `gzip_vary on` 会自动添加正确的单个 `Vary` 头部 |
...
`reuseport` 启用了 `SO_REUSEPORT`——内核将传入连接直接分发给 worker 进程，消除了 accept 互斥锁竞争。

## CDN 厂商头部

模拟器同时注入所有五大 CDN 厂商的头部。这允许 F5 XC 配置任意厂商的"可信客户端 IP 头部"，并且无论模拟的是哪个 CDN，都能看到真实的头部有效载荷。
...
| `X-Forwarded-Host` | 原始主机名 | 原始 Host 头部 |
| `X-Forwarded-Port` | 服务器端口 | 原始端口 |
| `X-Real-IP` | 客户端 IP | 单个客户端 IP（nginx 约定） |
| `Via` | `1.1 cdn-simulator` | 代理标识 |
| `Forwarded` | RFC 7239 格式 | 标准化转发头部 |
| `CDN-Loop` | `cdn-simulator` | 循环检测 |
...
| `CloudFront-Viewer-Country-Region` | `CA` | 区域代码 |
| `CloudFront-Viewer-Country-Region-Name` | `California` | 完整区域名称 |
...
| `X-Azure-RequestChain` | `hops=1` | 循环检测跳数 |
...
| 指标 | 值 |
|---|---|
| 峰值吞吐量（缓存命中） | 172,540 req/s |
| 持续吞吐量（缓存命中） | 85,000-103,000 req/s |
| 峰值连接数 | 15,000 并发 |
| 缓存命中率 | 100%（预热后） |
| 负载下内存 | 1.2 GB 稳定（16 GB 的 8%） |
| 峰值 CPU | 100%（4 核——CPU 是瓶颈） |
| 48 小时测试期间错误数 | 0 |
| 内存泄漏 | 未检测到 |
| 连接泄漏 | 未检测到 |
