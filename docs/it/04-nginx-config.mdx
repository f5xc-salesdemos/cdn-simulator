...
Il provisioning cloud-init distribuisce una configurazione NGINX CDN edge completamente ottimizzata per le prestazioni. Questa pagina documenta ogni livello di configurazione, dal tuning del kernel al comportamento della cache fino all'iniezione degli header dei vendor. Tutte le impostazioni sono state verificate durante un test di carico continuo di 48 ore con un picco di 172.540 req/s.

## File di configurazione

| File | Scopo |
|---|---|
| `/etc/sysctl.d/99-cdn-tuning.conf` | Tuning di rete del kernel Linux |
| `/etc/systemd/system/nginx.service.d/override.conf` | Limiti dei file descriptor per NGINX |
| `/etc/security/limits.d/99-nginx.conf` | Limiti a livello OS per l'utente www-data |
...
Applicato tramite `/etc/sysctl.d/99-cdn-tuning.conf` all'avvio.

| Parametro | Valore | Default | Scopo |
|---|---|---|---|
| `net.core.somaxconn` | 65535 | 4096 | Dimensione della coda di backlog del listen |
| `net.core.netdev_max_backlog` | 65535 | 1000 | Backlog dei pacchetti in ingresso per CPU |
| `net.ipv4.tcp_max_syn_backlog` | 65535 | 256 | Coda delle richieste SYN (previene la perdita di pacchetti sotto burst) |
| `net.ipv4.tcp_tw_reuse` | 1 | 2 | Riutilizzo dei socket TIME_WAIT per connessioni in uscita |
| `net.ipv4.ip_local_port_range` | 1024-65535 | 32768-60999 | Range delle porte effimere (64K vs 28K) |
| `net.core.rmem_max` | 16 MB | 212 KB | Buffer massimo del socket di ricezione |
| `net.core.wmem_max` | 16 MB | 212 KB | Buffer massimo del socket di invio |
...
| `net.ipv4.tcp_fin_timeout` | 15 | 60 | Timeout FIN_WAIT_2 (pulizia socket più rapida) |
| `net.ipv4.tcp_keepalive_time` | 300 | 7200 | Avvia i probe keepalive dopo 5 min di inattività |
...
| `fs.file-max` | 2097152 | variabile | Limite dei file descriptor a livello di sistema |
| `vm.swappiness` | 10 | 60 | Preferisce la RAM allo swap per i dati della cache |
...
L'override di Systemd in `/etc/systemd/system/nginx.service.d/override.conf` imposta `LimitNOFILE=65535` per coerenza.
...
La dimensione del busy buffer di 256k è critica — deve superare la risposta memorizzata in cache più grande (Juice Shop è 75 KB). L'impostazione originale di 64k causava serializzazione sotto carico.
...
Memorizza in cache i file descriptor e i metadati degli oggetti nella cache, eliminando le chiamate di sistema `stat()` e `open()` sui file più richiesti.
...
Il formato di log `cdn` include `$upstream_cache_status` (HIT/MISS) e `$request_time` per l'analisi della latenza. Il logging con buffer (256k/flush ogni 5s) riduce il sovraccarico I/O sotto alto carico.
...
Con 4 worker x 256 keepalive = 1.024 connessioni attive verso l'origin. Combinato con `proxy_http_version 1.1` e `proxy_set_header Connection ""`, questo elimina il sovraccarico dell'handshake TCP su ogni cache miss.
...
| `max_size=25g` | Limite disco di 25 GB | Utilizza la maggior parte del disco OS da 30 GB; eviction LRU al raggiungimento del limite |
| `inactive=24h` | Timeout di inattività di 24 ore | Il contenuto non acceduto per 24h viene rimosso (non è lo stesso del TTL) |
| `use_temp_path=off` | Scrittura diretta nella directory della cache | Evita gli spostamenti di file tra filesystem diversi |
...
| `proxy_cache_lock_age 3s` | Timeout del lock | Se la prima richiesta non è completata in 3s, ne viene fatta passare un'altra |
| `proxy_cache_background_update on` | Aggiornamento a latenza zero | Serve immediatamente il contenuto stale mentre l'aggiornamento avviene in background |
...
| `proxy_ignore_headers` | Forzare il caching | Ignora Set-Cookie, Cache-Control, Expires, Vary dell'origin — il CDN decide il comportamento di TTL e Vary (Juice Shop invia `max-age=0` e tripli header `Vary` che impedivano un caching efficace) |
| `proxy_hide_header X-Cache-Status` | Rimuove l'header dell'origin | L'NGINX dell'origin aggiunge il proprio X-Cache-Status — viene rimosso affinché sia visibile solo lo stato della cache del CDN |
| `proxy_hide_header Vary` | Previene la frammentazione della cache | L'origin invia molteplici header `Vary: Accept-Encoding`. Rimuoverli previene la frammentazione delle chiavi di cache tra le permutazioni di `Accept-Encoding`. La direttiva `gzip_vary on` di NGINX aggiunge automaticamente il singolo header `Vary` corretto |
...
Concede all'origin più tempo di risposta sotto carico pur fallendo rapidamente in caso di problemi di connessione.
...
Il simulatore inietta header di tutti e cinque i principali vendor CDN simultaneamente. Questo consente di configurare F5 XC con il "Trusted Client IP Header" di qualsiasi vendor e di vedere payload di header realistici indipendentemente dal CDN simulato.

### Header standard del settore (tutti i CDN)
...
| `X-Akamai-Edgescape` | Stringa geo composita | georegion, country_code, region_code, city, dma, pmsa, msa, areacode, county, fips, lat, long, timezone, zip, continent, throughput, bw, network, asnum, network_type |
...
| Header | Valore | Scopo |
|---|---|---|
| `CF-Connecting-IP` | IP del client | Vero IP del client (sempre presente) |
| `CF-IPCountry` | `US` | Codice paese a due lettere |
| `cf-ipcity` | `San Jose` | Città del client |
| `cf-ipcontinent` | `NA` | Codice del continente |
...
| `cf-metro-code` | `807` | Codice metro US |
...
| `cf-verified-bot` | `false` | Flag di bot verificato |
...
| Header | Valore | Scopo |
|---|---|---|
| `CloudFront-Viewer-Address` | `IP:port` | IP del client e porta sorgente |
...
| `CloudFront-Viewer-Metro-Code` | `807` | Codice metro US |
...
| Header | Valore | Scopo |
|---|---|---|
| `Fastly-Client-IP` | IP del client | Vero IP del client |
...
| `X-Geo-Metro-Code` | `807` | Codice metro US |
...
| `X-Azure-RequestChain` | `hops=1` | Conteggio degli hop per il rilevamento dei loop |
...
- **Desktop**: Predefinito quando né mobile né tablet corrispondono

Il tipo di dispositivo è riflesso in:
...
Oppure aggiornare `origin_host` in `terraform.tfvars` ed eseguire `terraform apply` per il reprovisioning.
...
Verificati durante un test di carico continuo di 48 ore:

| Metrica | Valore |
|---|---|
| Throughput di picco (con cache) | 172.540 req/s |
| Throughput sostenuto (con cache) | 85.000-103.000 req/s |
| Connessioni di picco | 15.000 concorrenti |
| Rapporto di cache hit | 100% (a cache attiva) |
