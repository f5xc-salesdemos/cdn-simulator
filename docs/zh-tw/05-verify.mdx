...
部署完成後，請執行以下檢查以確認 CDN 邊緣節點運作正常。在執行 `terraform apply` 後，請等待 2-3 分鐘讓 cloud-init 完成 NGINX 的安裝。
...
`HIT` 確認回應是從 NGINX 磁碟快取中提供的，無需聯繫來源伺服器。

## 快取目錄檢查

透過 SSH 登入 VM 以驗證快取內容存在於磁碟上：
...
2. 來源伺服器是否允許來自邊緣節點公共 IP 的入站連線
3. VM 上的 DNS 解析是否正常（`nslookup your-origin.example.com`）
...
透過請求 `/headers` 路徑來驗證所有 CDN 供應商標頭是否已被注入（當使用 httpbin.org 作為來源伺服器時）：
...
使用行動裝置 User-Agent 進行測試，以驗證裝置偵測標頭是否會變更：
...
執行 `terraform apply` 後，cloud-init 需要 2-3 分鐘。檢查進度：
...
- 檢查來源伺服器是否未發送 `Cache-Control: no-cache` 或 `no-store` 標頭
- 檢查 NGINX 錯誤日誌：`sudo tail -f /var/log/nginx/error.log`

### 連接埠 80/443 連線被拒絕

- 驗證 NSG 規則已套用：`az network nsg rule list --nsg-name "$(terraform output -raw nsg_name)" --resource-group "$(terraform output -raw resource_group_name)" -o table`
- 驗證 NGINX 正在監聽：`ssh azureuser@<PUBLIC_IP> "sudo ss -tlnp | grep nginx"`
