...
Após a implantação, execute estas verificações para confirmar que o nó de borda do CDN está funcionando corretamente. Aguarde 2-3 minutos após o `terraform apply` para que o cloud-init termine a instalação do NGINX.
...
Se estiver usando as saídas do Terraform:
...
A primeira requisição para qualquer caminho será um cache miss — o nó de borda busca do servidor de origem:

```bash
curl -I "http://<PUBLIC_IP>/"
```

Procure por estes cabeçalhos na resposta:
...
`MISS` significa que o conteúdo não estava no cache e foi buscado do servidor de origem.
...
`HIT` confirma que a resposta foi servida a partir do cache em disco do NGINX sem contatar o servidor de origem.

## Inspeção do Diretório de Cache

Conecte-se via SSH na VM para verificar se o conteúdo em cache existe no disco:
...
# Segunda requisição — HIT (servido do cache)
...
## Verificação de Cabeçalhos de Fornecedores

Verifique se todos os cabeçalhos de fornecedores CDN estão sendo injetados, requisitando o caminho `/headers` (ao usar httpbin.org como origem):
...
Teste com um User-Agent de dispositivo móvel para verificar se os cabeçalhos de detecção de dispositivo mudam:
