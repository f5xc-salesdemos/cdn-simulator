...
title: 검증
description: >-
  예상 HTTP 응답 코드, X-Cache-Status 헤더 값(HIT/MISS), 상태 확인 JSON 응답, 캐시 디렉터리 검사, 오리진
  연결 확인, cloud-init, NGINX 시작 및 NSG 문제 해결을 위한 스모크 테스트 명령
sidebar:
  order: 5
i18n:
...
`MISS`는 콘텐츠가 캐시에 없어 오리진 서버에서 가져왔음을 의미합니다.
